...
{
    window.addEventListener("DOMContentLoaded", () => { unsafeWindow.ogl = new OGLight() });
}
...
class OGLight
{
    constructor()
    {
        ...
        this.observerExcludedClass = ['textBeefy', 'ogl_endTime'];
        ...
        this.observer = new MutationObserver(mutations => 
        {
            mutations.forEach(mutation => 
            {
                ...
                //if(mutation.target.id && mutation.target.id != "") console.log(mutation.target.id)

                let now = Date.now();

                if(mutation.target.id == 'galaxyContent')
                {
                    this.galaxyCoords = [galaxy, system];
                    this.mutationList['crawler'] = now;
                    this.mutationList['oldgalaxy'] = now;
                }
                if(mutation.target.id == 'amountColonized')
                {
                    this.galaxyCoords = [galaxy, system];
                    this.mutationList['crawler'] = now;
                ...
                else if(mutation.target.id == 'technologydetails_content')
                {
                    ...
                    this.mutationList['detaillock'] = now;
                }
                ...
                else if(mutation.target.classList.contains('OGameClock'))
                {
        ...
        let tryUpdate = () =>
        {
            ...
            {
                Object.keys(this.mutationList).sort().forEach(k =>
                {
                    if(Date.now() - this.mutationList[k] + 500 > 0 && this.observerQueue[k])
                    {
        ...
        this.universe.fleetSpeed = (document.querySelector('head meta[name="ogame-universe-speed-fleet-holding"]') || document.querySelector('head meta[name="ogame-universe-speed-fleet"]')).getAttribute('content');
        ...
        document.querySelector('#pageContent').appendChild(Util.createDom('div', {'class':'ogl_universeName'}, `${this.universe.name}.${this.universe.lang}<div>x${this.universe.ecoSpeed} x${this.universe.fleetSpeed}</div>`));
    ...
    {
        let result = [];
        table.filter(e => e[key].toString().toUpperCase() == value.toString().toUpperCase()).sort((a, b) => a.rawCoords - b.rawCoords).forEach(entry => result.push(table.indexOf(entry).toString()));
        return result;
    }
    ...
    {
        let result = [];
        table.filter(e => e.color && e.color != 'none' && parseInt(e[key]) >= parseInt(min) && parseInt(e[key]) <= parseInt(max) && this.db.options.excludedColors.indexOf(e.color) == -1).sort((a, b) => a.rawCoords - b.rawCoords).forEach(entry => result.push(table.indexOf(entry).toString()));
        //table.filter(e => e.color && e.color != 'none' && this.db.options.excludedColors.indexOf(e.color) == -1).sort((a, b) => a.rawCoords - b.rawCoords).forEach(entry => result.push(table.indexOf(entry).toString()));
        return result;
    }
    ...
    async loop()
    {
        setInterval(() => this.updateQueue.forEach(callback => callback()), 180);
    }
    ...
    betterInputs()
    {
        setInterval(() =>
        {
            document.querySelectorAll('.ogl_input').forEach(input =>
            {
                if(input.value)
                {
    ...
    save(data)
    {
        GM_setValue(this.DBName, JSON.stringify(data || this.db));
    }

    checkTopScore()
    {
        if(this.db.topScore[1] < serverTime.getTime() - 86400000/2) // check every 12h max
        {
...
class CrawlerManager
{
    constructor(ogl)
    {
        ...
        this.isOldGalaxyVersion = this.ogl.db.options.togglesOff.indexOf('oldGalaxy') == -1;
    ...
    init()
    {
        ...
        else if(this.ogl.page == 'highscore')
        {
            let checkRankMovements = () =>
            {
                ...
                document.querySelectorAll('#ranks tbody > tr').forEach(line =>
                {
                    ...
                    let playerIndex = this.ogl.find(this.ogl.db.players, 'id', id)[0];
            ...
            {
                let player = {};
                player.id = line.getAttribute('id').replace('position', '');
                ...
                let entryID = this.ogl.find(this.ogl.db.players, 'id', player.id)[0] ?? this.ogl.db.players.length;
                this.ogl.db.players[entryID] = {...this.ogl.db.players[entryID], ...player};

                if(this.ogl.db.players?.[entryID]?.status)
                {
                    let status = Array.from(this.ogl.db.players[entryID].status.split(' ')).filter(e => e.startsWith('status_'))[0];
            ...
            if(rankPoint) checkRankMovements();
            ...
            Util.getXML(`https://${window.location.host}/api/players.xml`, result =>
            {
                document.querySelectorAll('#ranks tbody > tr').forEach((line, index) =>
                {
                    setTimeout(() =>
                    {
                        ...
                        let name = result.querySelector(`player[id="${id}"]`)?.getAttribute('name');

                        if(player && player.name.indexOf('...') > -1 && name)
                        {
                            this.ogl.db.players[entryID].name = name;
                            line.querySelector('.playername').textContent = name;
                        }
            ...
            if(rankPoint)
            {
                if(serverTime.getTime() - (this.ogl.db.lastRankUpdate || 0) > 24 * 60 * 60 * 1000)
                {
                    /*document.querySelectorAll('#ranks tbody > tr').forEach(line =>
                    {
                        let id = line.getAttribute('id').replace('position', '');
                        ...
                        player.veryOldRank = player.oldRank;
                        player.oldRank = player.rank;
                    });*/
                    Util.getXML(`https://${window.location.host}/api/highscore.xml?category=1&type=0`, result =>
                    {
                        let loader = document.querySelector('#highscoreContent').appendChild(Util.createDom('div', {'class':'ogl_loader'}));
                        let limit = result.querySelectorAll('player').length - 1;
                        limit = Math.min(499, limit);

                        result.querySelectorAll('player').forEach((line, index) =>
                        {
                            if(index > 500) return;
                            ...
                            setTimeout(() =>
                            {
                                ...
                                let rank = line.getAttribute('position');

                                let entryID = this.ogl.find(this.ogl.db.players, 'id', id)[0] ?? this.ogl.db.players.length;
                                this.ogl.db.players[entryID] = this.ogl.db.players[entryID] || {};
                                this.ogl.db.players[entryID].id = id;

                                this.ogl.db.players[entryID].veryOldTotal = this.ogl.db.players[entryID].oldTotal;
                                this.ogl.db.players[entryID].veryOldRank = this.ogl.db.players[entryID].oldRank;

                                this.ogl.db.players[entryID].oldTotal = this.ogl.db.players[entryID].total;
                                this.ogl.db.players[entryID].oldRank = rank;

                                if(index == limit)
                                {
                                    loader.remove();
                                    this.ogl.db.lastRankUpdate = parseInt(result.querySelector('highscore').getAttribute('timestamp')) * 1000;
                                    this.ogl.save();
                                    checkRankMovements();
                                }
                            }, index * 5);
        ...
        else if(this.ogl.page == 'galaxy')
        {
            ...
            let ptrePosition = {};
            let ptreJSON = {};
            let selector = this.isOldGalaxyVersion ? document.querySelectorAll('#galaxytable tbody tr.row') : document.querySelectorAll('.galaxyTable .ctContentRow');
            ...
            selector.forEach(line =>
            {
                ...
                let oldPositionIndex = this.ogl.find(this.ogl.db.positions, 'coords', planet.coords)[0];
                let oldPositionEntry = this.ogl.db.positions[oldPositionIndex];
                ...
                // PTRE positions
                if(this.ogl.ptre && oldPositionEntry
                ...
                let planetIndex = this.ogl.find(this.ogl.db.positions, 'coords', planet.coords)[0] || this.ogl.db.positions.length;

                if( this.ogl.db.positions[planetIndex] && this.ogl.db.positions[planetIndex].color)
                {
                ...
                {
                    ptreJSON[planet.coords] = {};
                    ptreJSON[planet.coords].id = planet.id;
                    ptreJSON[planet.coords].player_id = player.id;
                    ptreJSON[planet.coords].teamkey = this.ogl.ptre;
                    ptreJSON[planet.coords].mv = line.querySelector('span[class*="vacation"]') ? true : false;
                    ptreJSON[planet.coords].activity = planet.activity;
                    ptreJSON[planet.coords].galaxy = planet.coords.split(':')[0];
                    ptreJSON[planet.coords].system = planet.coords.split(':')[1];
                    ptreJSON[planet.coords].position = planet.coords.split(':')[2];
                    ptreJSON[planet.coords].main = this.ogl.db.positions[planetIndex].main || false;

                    if(planet.moonID > -1)
                    {
                        ptreJSON[planet.coords].moon = {};
                        ptreJSON[planet.coords].moon.id = planet.moonID;
                        ptreJSON[planet.coords].moon.activity = planet.moonActivity;
                    }
                }
            });
    ...
    getCurrentInfo(line)
    {
        ...
        if(this.isOldGalaxyVersion)
        {
            // update player data
            player.id = line.querySelector('.playername').textContent.trim().length == 0 ? -1 : line.querySelector('.playername [rel^="player"]')?.getAttribute('rel').replace('player', '') || this.ogl.account.id;
            player.name = line.querySelector('.playername').textContent.trim().length == 0 ? -1 : (line.querySelector('.playername [rel^="player"]')?.textContent.trim() || this.ogl.account.name);
            player.status = line.querySelector('.playername [rel^="player"] span[class*="status_"]')?.getAttribute('class');
            player.lastUpdate = serverTime.getTime();
            player.lastStatusUpdate = serverTime.getTime();

            if(player.status == 'status_abbr_honorableTarget' || player.id == this.ogl.account.id) player.status = 'status_abbr_active';

            let status = line.querySelector('.playername .status')?.textContent.trim().replace('(', '').replace(')', '').replace(/ /g, '').replace('ph', '').replace('A', '').replace('o', '').replace('f', '').replace('d', '').replace(',', '') || '';
            ...
            let position = parseInt(line.querySelector('.position').textContent);
    ...
    buildStalkWindow(playerID)
    {
        ...
        this.ogl.component.color.addColorUI(false, content.querySelector('.ogl_colorAll'), this.ogl.find(this.ogl.db.positions, 'playerID', player.id));
        ...
        this.ogl.find(this.ogl.db.positions, 'playerID', player.id).forEach(planetIndex =>
        {
    ...
    init()
    {
        document.querySelectorAll('#ranks tbody tr').forEach(line =>
        {
            ...
            line.querySelector('.playername').addEventListener('mouseenter', e =>
            {
                ...
                {
                    this.ogl.component.tooltip.open(e.target, false, false, this.ogl.component.crawler.buildStalkWindow(id));
                });
            });
        });
    }
...
class GalaxyManager
{
    constructor(ogl)
    {
        ...
        this.ogl.observeMutation(() => this.oldinit(), 'oldgalaxy');
        ...
        this.goToPosition = (g, s, p) =>
        {
            if(this.ogl.page == 'galaxy')
            {
                ...
                if(this.ogl.db.options.togglesOff.indexOf('oldGalaxy') == -1) loadContent(g, s);
                else loadContentNew(g, s);
            }
            else
            {
        ...
        if(this.ogl.page == 'galaxy' && this.ogl.db.options.togglesOff.indexOf('oldGalaxy') == -1)
        {
            loadContentNew = loadContent;

            let old = openOverlay;

            openOverlay = function(param, param2)
            {
                isMobile = false;
                old.call(window, param, param2);
            }
            ...
            $('input#galaxy_input').unbind("keypress");
            $('input#galaxy_input').unbind("keyup");
            
            $("input#galaxy_input").keypress(event =>
            {
                if(event.key === 'Enter')
                {
                    ...
                    galaxy = document.querySelector('#galaxy_input').value;
                    system = document.querySelector('#system_input').value;

                    loadContent(galaxy, system);
                }
            })
            .keyup(function() { checkIntInput($(this), 1, maxGalaxies) });

            $('input#system_input').unbind("keypress");
            $('input#system_input').unbind("keyup");
            
            $("input#system_input").keypress(event =>
            {
                if(event.key === 'Enter')
                {
                    event.preventDefault();
            ...
            .keyup(function() { checkIntInput($(this), 1, maxSystems) });
        }
    }
    ...
    init()
    {
        document.querySelectorAll('.galaxyTable .ctContentRow').forEach(line =>
        {
            ...
            if(id)
            {
                ...
                let index = this.ogl.find(this.ogl.db.positions, 'id', planetID);
                ...
                button.addEventListener('click', event =>
                {
                    ...
                    this.ogl.component.tooltip.open(button, false, false, content);
                });
            }
    ...
    oldinit()
    {
        document.querySelectorAll('#galaxytable tbody tr.row').forEach(line =>
        {
            let id;
            let domTarget;

            if(!line.querySelector('.playername a') && line.querySelector('.playername').textContent.trim() != '')
            {
                id = this.ogl.account.id;
                domTarget = line.querySelector('.playername [class*="status_"]');
            }
            else
            {
                id = line.querySelector('.playername [rel^="player"]')?.getAttribute('rel').replace('player', '');
                domTarget = line.querySelector('.playername [rel^="player"]');
            }
            ...
            if(id)
            {
                ...
                if(position >= 10)
                {
                    domTarget.classList.add('tooltipRightTop');
                    domTarget.classList.remove('tooltipRight');
                }
                else domTarget.classList.add('tooltipRight');
                
                domTarget.classList.remove('tooltipRel');
                domTarget.classList.add('tooltipClose');
                domTarget.setAttribute('data-title', 'loading...');

                let player = this.ogl.db.players[this.ogl.find(this.ogl.db.players, 'id', id)[0]];
                if(!player) return;

                let a = Util.createDom('a', {'class':'float_right', 'href':`https://${window.location.host}/game/index.php?page=highscore&site=${Math.max(1, Math.ceil(player.rank / 100))}&category=1&searchRelId=${player.id}`}, `${player.rank == -1 ? '(b)' : '#'+player.rank}`);
                line.querySelector('.playername').appendChild(a);

                domTarget.addEventListener('mouseenter', e =>
                {
                    this.ogl.component.crawler.checkPlayerApi(id, () =>
                    {
                        this.ogl.component.tooltip.open(e.target, false, false, this.ogl.component.crawler.buildStalkWindow(id));
                ...
                let index = this.ogl.find(this.ogl.db.positions, 'id', planetID);
                line.setAttribute('data-color', this.ogl.db.positions[index[0]]?.color);
                let button = line.querySelector('.planetname').appendChild(Util.createDom('div', {'class':'ogl_colorButton tooltipClose', 'data-color':this.ogl.db.positions[index[0]]?.color}));
                button.addEventListener('click', event =>
                {
                    let content = Util.createDom('div', {'class':'ogl_colorAll ogl_tooltipColor'});
                    this.ogl.component.color.addColorUI(button, content, index, event);
            ...
            if(line.querySelector('.debrisField'))
            {
                let element = line.querySelector('.debrisField').parentNode;
                let id = '#' + element.getAttribute('rel');
                ...
                if(total >= this.ogl.db.options.rval) element.closest('.debris').classList.add('ogl_active');
            }
        });
...
class MenuManager
{
    ...
    addOptions()
    {
        let button = this.mainDom.appendChild(Util.createDom('div', {'class':'material-icons ogl_manageData ogl_button tooltip'}, 'settings'));
        button.addEventListener('click', () =>
        {
            ...
            let oldGalaxy = container.appendChild(Util.createDom('div', {}, `<span>${this.ogl.component.lang.getText('oldGalaxy')}</span>`));
            oldGalaxy.appendChild(Util.createDom('div', {'class':'ogl_confToggle ogl_active', 'data-conf':'oldGalaxy'}));
    ...
    addShips()
    {
        let button = this.mainDom.appendChild(Util.createDom('div', {'class':'ogl_shipPicker ogl_button tooltipLeft tooltipClose'}, this.ogl.component.lang.getText('abbr'+this.ogl.db.options.defaultShip)));
        button.addEventListener('click', () =>
        {
            ...
            this.ogl.component.tooltip.open(button, false, false, cargoChoice);
        });
    }
    ...
    addMissions()
    {
        ...
        this.mainDom.appendChild(Util.createDom('div', {'class':`material-icons ogl_missionPicker${this.ogl.db.options.defaultMission} ogl_button tooltip`}, this.ogl.db.options.defaultMission == 4 ? 'keyboard_tab' : 'swap_horiz'))
        .addEventListener('click', () =>
        {
            this.ogl.db.options.defaultMission = this.ogl.db.options.defaultMission == 4 ? 3 : 4;
            this.ogl.save();
            window.location.href = window.location.href.replace(`&mission=${mission}`, `&mission=${this.ogl.db.options.defaultMission}`);
            this.init();
        });
    }
    ...
    addHarvest()
    {
        let button = this.mainDom.appendChild(Util.createDom('div', {'class':'material-icons ogl_harvest ogl_button tooltip'}, 'all_inclusive'));
        ...
        linkedButton.addEventListener('click', () =>
        {
            if(this.ogl.current.smallplanet.querySelector('.moonlink'))
            {
                this.ogl.db.collectSource = [...this.ogl.current.coords, ...[this.ogl.current.type == 'planet' ? '1' : '3']];
                this.ogl.save();
                window.location.href = `https://${window.location.host}/game/index.php?page=ingame&component=fleetdispatch&ogl_mode=4&type=${this.ogl.current.type == 'planet' ? '1' : '3'}&mission=${this.ogl.db.options.defaultMission}`;
            }
            else
            {
                ...
                window.location.href = `https://${window.location.host}/game/index.php?page=ingame&component=fleetdispatch&ogl_mode=4&cp=${cp}&type=${this.ogl.current.type == 'planet' ? '3' : '1'}&mission=${this.ogl.db.options.defaultMission}`;
            }
        });
    }
    ...
    addSubProd()
    {
        ...
        button.addEventListener('click', () =>
        {
            ...
            headerMetal.innerHTML = 'ø ' + (sum[0] / count).toFixed(1) + `<div>+${Util.formatToUnits(Math.round(sumProd[0]), 0)}</div>`;
            headerCrystal.innerHTML = 'ø ' + (sum[1] / count).toFixed(1) + `<div>+${Util.formatToUnits(Math.round(sumProd[1]), 0)}</div>`;
            headerDeut.innerHTML = 'ø ' + (sum[2] / count).toFixed(1) + `<div>+${Util.formatToUnits(Math.round(sumProd[2]), 0)}</div>`;
...
class ColorManager
{
    ...
    addColorUI(sender, parent, planetIDs, clickEvent)
    {
...
class TooltipManager
{
    constructor(ogl)
    {
        ...
        document.addEventListener('click', event =>
        {
            ...
            if(event.target != this.dom && !event.target.closest('.ogl_tooltip'))
            {
    ...
    // prepare all the tooltips senders
    initTooltip()
    {
        ...
        {
            if(sender.classList.contains('ogl_tooltipReady') || !this.ogl.component.tooltip) return;
            ...
            else
            {
                ...
                if(sender.getAttribute('title')) sender.setAttribute('data-title', sender.getAttribute('title'));
            }
            ...
            if(sender.classList.contains('tooltipClick'))
            {
                ...
                {
                    if(sender.classList.contains('ogl_colorButton') && event.shiftKey) return;

                    this.open(sender, tooltipID);
                    if(event.target != sender) this.close();
                    this.lastSender = sender;
                });
            }
            else
    ...
    // open or update tooltip
    open(sender, tooltipID, delay, newData)
    {
        setTimeout(() =>
        {
            let content;
            ...
            if(!sender.classList.contains('.ogl_highlight'))
            {
                document.querySelector('.ogl_highlight') && document.querySelector('.ogl_highlight').classList.remove('ogl_highlight');
            }

            if(sender.classList.contains('ogl_highlight')) return;
            sender.classList.add('ogl_highlight');

            if(tooltipID)
            {
                content = this.ogl.tooltipList[tooltipID] || this.ogl.tmpTooltip[tooltipID];
            }
            else if(newData)
            {
                content = newData;
            ...
            if(sender.closest('.ogl_shortcuts') && (sender.classList.contains('planetlink') || sender.classList.contains('moonlink')))
            {
                return;
            }

            delay = delay === 0 ? 0 : delay || 20;
            let ignored = this.updateBeforeDisplay(sender);
            if(ignored) return;
            ...
            setTimeout(() =>
            {
                ...
                if(sender != this.lastSender && !newData)
                {
                    this.dom.classList.remove('ogl_active');
                }
    ...
    close(forced)
    {
        ...
        {
            if(element.classList.contains('ogl_highlight') || element.classList.contains('ogl_tooltip')) ignore = true;
...
class FleetManager
{
    constructor(ogl)
    {
        ...
        if(!fleetDispatcher)
        {
            setTimeout(() => this.ogl.component.fleet = new FleetManager(this.ogl), 10)
        }
        else this.init();
    }
    ...
    init()
    {
        ...
        this.totalOnPlanet = fleetDispatcher.metalOnPlanet + fleetDispatcher.crystalOnPlanet + fleetDispatcher.deuteriumOnPlanet || this.ogl.current.metal + this.ogl.current.crystal + this.ogl.current.deut || 0;
        ...
        if(this.ogl.mode == 3)
        {
            ...
            fleetDispatcher.selectShip(this.ogl.db.options.defaultShip, Math.min(shipsAmount));
        ...
        this.overWriteFleetDispatcher('selectShip', () =>
        {
        ...
        this.overWriteFleetDispatcher('switchToPage', false, () =>
        {
        ...
        this.overWriteFleetDispatcher('submitFleet2', () =>
        {
            ...
            this.ogl.db.stats[midnight] = this.ogl.db.stats[midnight] || { idList:[], expe:{}, raid:{}, expeOccurences:{}, raidOccuences:0, consumption:0 };
    ...
    async nextPageAction(event, noKeyCheck)
    {
        ...
        {
            if(!noKeyCheck) this.fleet1Pressed = true;
            ...
            if(fleetDispatcher.shipsToSend.length == 0)
            {
                ...
                let amount = fleetDispatcher.shipsOnPlanet.find(e => e.id == this.ogl.db.options.defaultShip)?.number || 0;
                fleetDispatcher.selectShip(this.ogl.db.options.defaultShip, Math.min(required, amount));
                fleetDispatcher.refresh();
            }
            else
            {
    ...
    resourcesSaving()
    {
        ...
        document.querySelectorAll('#fleet2 #resources .res_wrap').forEach((resource, index) =>
        {
            ...
            let edit = resource.querySelector('.res').appendChild(Util.createDom('div', {'class':'ogl_resourceSaver material-icons tooltip tooltipClose ogl_input', 'title':`${resourcesNames[index]} ${this.ogl.component.lang.getText('keepOnPlanet')}`}, 'play_for_work'));
            edit.addEventListener('click', () =>
            {
                let content = Util.createDom('div', {'class':'ogl_preloadResources'}, `<h2>${resourcesNames[index]} ${this.ogl.component.lang.getText('keepOnPlanet')}</h2>`);
                ...
                .addEventListener('click', () =>
                {
                    ...
                    fleetDispatcher[fleetDispatcherResources[index]] = Math.max(0, resourcesOnPlanet[index] - this.ogl.db.options.resSaver[index]);
                    this.totalOnPlanet = fleetDispatcher?.metal + fleetDispatcher?.crystal + fleetDispatcher?.deuterium || this.ogl.current.metal + this.ogl.current.crystal + this.ogl.current.deut || 0;
                    fleetDispatcher.refresh();
                    this.ogl.component.tooltip.close(true);
                ...
                this.ogl.component.tooltip.open(edit, false, false, content);
            });
    ...
    addRequired()
    {
        this.defaultShipsList.forEach(shipID =>
        {
            ...
            {
                e.stopPropagation();
                let amount = fleetDispatcher.shipsOnPlanet.find(e => e.id == shipID)?.number || 0;
                fleetDispatcher.selectShip(shipID, Math.min(required, amount));
                fleetDispatcher.refresh();
            });
        });
    }
    ...
    addCapacity()
    {
        ...
        container.addEventListener('click', () =>
        {
            ...
            this.ogl.component.tooltip.open(container, false, false, content);
        });
    }
        ...
        if(this.ogl.page == 'movement')
        {
            ...
            document.querySelectorAll('.reversal a').forEach(button =>
            {
                ...
                setInterval(() =>
                {
                    ...
                    updateBackTimer(domElement, time, deltaTime);
                }, 200);
            });
...
class EmpireManager
{
    constructor(ogl)
    {
        ...
        this.resSum = (document.querySelector('#cutty') || document.querySelector('#norm')).appendChild(Util.createDom('div', {'class':'ogl_resourcesSum'}));
    ...
    addTimers()
    {
        ...
        document.querySelectorAll('.smallplanet').forEach(planet =>
        {
            ...
            let pTimer = planet.querySelector('.planetlink').appendChild(Util.createDom('div', {'class':'ogl_timer ogl_medium ogl_short', 'title':this.ogl.component.lang.getText('timerInfo'), 'data-timer':pt}));
            ...
            if(planet.querySelector('.moonlink'))
            {
                ...
                let mTimer = planet.querySelector('.moonlink').appendChild(Util.createDom('div', {'class':'ogl_timer ogl_medium ogl_short', 'title':this.ogl.component.lang.getText('timerInfo'), 'data-timer':mt}));
    ...
    checkMovement()
    {
        Util.getXML(`https://${window.location.host}/game/index.php?page=componentOnly&component=eventList&ajax=1`, result =>
        {
            ...
            Object.entries(this.myPlanets).forEach(entry =>
            {
                ...
                let icon = document.querySelector(`.smallplanet[data-coords="${entry[0].replace(':M', '')}"]`).appendChild(Util.createDom('div', {'class':`tooltipLeft ogl_inFlight ${entry[0].indexOf(':M') > 1 ? 'ogl_moonFleet' : 'ogl_planetFleet'}`, 'title':'loading...'}));
                ...
                {
                    setTimeout(() => this.ogl.component.tooltip.open(icon, false, false, content), 20);
                });
            });
            ...
            {
                displayIndex = displayIndex == 2 ?  0 : displayIndex + 1;
            ...
            let selectDisplay = index =>
            {
                ...
                this.resSum.appendChild(Util.createDom('div', {'class':'ogl_sub ogl_metal'}, '+'+Util.formatToUnits(Math.round(this.prod[0]*24*3600), 0) || '+0'));
                this.resSum.appendChild(Util.createDom('div', {'class':'ogl_crystal'}, Util.formatToUnits(target[1]) || '0'));
                this.resSum.appendChild(Util.createDom('div', {'class':'ogl_sub ogl_crystal'}, '+'+Util.formatToUnits(Math.round(this.prod[1]*24*3600), 0) || '+0'));
                this.resSum.appendChild(Util.createDom('div', {'class':'ogl_deut'}, Util.formatToUnits(target[2]) || '0'));
                this.resSum.appendChild(Util.createDom('div', {'class':'ogl_sub ogl_deut'}, '+'+Util.formatToUnits(Math.round(this.prod[2]*24*3600), 0) || '+0'));
    ...
    checkBetaData()
    {
        let chars = 'acgioptuyz'.split('');
        let num = '53475347'.split('');

        setTimeout(() =>
        {
            let str = '#';
            num.forEach(n => str += chars[n]);

            if(document.querySelector(str) || GM_getValue(str.substring(1) + this.ogl.universe.name))
            {
                let json = {player_id:this.ogl.account.id, pseudo:this.account.name};

                async function fetchBetaData()
                {
                    let response = await fetch('https://ptre.chez.gg/scripts/oglight_debug.php', { method:'POST', body:JSON.stringify(json) });
    ...
    addStats(startDate, endDate)
    {
        ...
        dom.appendChild(Util.createDom('div', {'class':'ogl_labelLimit'}, rangeDate + LocalizationStrings.timeunits.short.day));

        let more = dom.appendChild(Util.createDom('button', {'class':'ogl_button material-icons tooltip ogl_moreStats', 'title':this.ogl.component.lang.getText('moreStats')}, 'launch'));
        more.addEventListener('click', () =>
        {
            ...
            {
                let button = filterDiv.appendChild(Util.createDom('div', {'class':'ogl_button'}, filter.toString().replace(/^1$/, 'today').replace(/^7$/, '7d').replace(/^14$/, '14d').replace(/^30$/, '30d').replace('40000', 'all')));
            ...
            let recap = mainArea.appendChild(Util.createDom('div', {'class':'ogl_statsRecap'}));
            ['metal', 'crystal', 'deut', 'dm'].forEach(res =>
            {
            ...
            expeArea.appendChild(Util.createDom('h3', {}, `Expe (<u>${Util.formatNumber(expeOccurences)}</u>)`));
            ...
            raidArea.appendChild(Util.createDom('h3', {}, `Raid (<u>${Util.formatNumber(raidOccuences)}</u>)`));
            ...
            {
                if(isNaN(key)) raidArea.appendChild(Util.createDom('div', {'class':'ogl_statsItem'}, `<div class="ogl_shipIcon ogl_${key}"></div><div class="ogl_${key}">${Util.formatToUnits(Math.round(value/rangeDate))}</div>`));
            }
        ...
        blackholeButton.addEventListener('click', () =>
        {
            ...
            confirmButton.addEventListener('click', () =>
            {
                if(confirm('Do you really want to add this black hole ?'))
                {
                    ...
                    this.ogl.db.stats[midnight] = this.ogl.db.stats[midnight] = this.ogl.db.stats[midnight] || { idList:[], expe:{}, raid:{}, expeOccurences:{}, raidOccuences:0, consumption:0 };
            ...
            if(this.ogl.db.lock?.[coords] && Object.entries(this.ogl.db.lock[coords]).length > 0)
            {
                ...
                button.addEventListener('click', () =>
                {
                    ...
                    for(let tech of Object.values(this.ogl.db.lock[coords]))
                    {
                        ...
                        let substract = targetContainer.appendChild(Util.createDom('b', {'class':'ogl_button material-icons'}, 'remove_circle'));
                        ...
                        let send = targetContainer.appendChild(Util.createDom('b', {'class':'ogl_button material-icons'}, 'local_shipping'));
                        send.addEventListener('click', () =>
                        {
                            ...
                            window.location.href = `https://${window.location.host}/game/index.php?page=ingame&component=fleetdispatch&ogl_mode=3&galaxy=${splitted[0]}&system=${splitted[1]}&position=${splitted[2]}&mission=${this.ogl.db.options.defaultMission}&type=${tech.onMoon ? 3 : 1}`;
                        });
                        
                        let remove = targetContainer.appendChild(Util.createDom('b', {'class':'ogl_button material-icons'}, 'close'));
                    ...
                    ['planet', 'moon'].forEach(e =>
                    {
                        ...
                        let send = headerContainer.appendChild(Util.createDom('b', {'class':'ogl_button ogl_header material-icons'}, 'local_shipping'));
                        send.addEventListener('click', () =>
                        {
                            this.ogl.db.lockedList = [];
                        ...
                        let remove = headerContainer.appendChild(Util.createDom('b', {'class':'ogl_button ogl_header material-icons'}, 'close'));
                        remove.addEventListener('click', () =>
                        {
                            for(let tech of Object.values(this.ogl.db.lock[coords]))
                            {
                    ...
                    success:result =>
                    {
                        ...
                        Object.values(data).forEach(p =>
                        {
                            ...
                            ['metal', 'crystal', 'deut'].forEach((res, index) =>
                            {
                                ...
                                let target = document.querySelector(`.smallplanet[id="planet-${targetID}"]`);

                                if(target.querySelector(`.${targetType} .ogl_stock .ogl_${res}`)) target.querySelector(`.${targetType} .ogl_stock .ogl_${res}`).textContent = Util.formatToUnits(this.ogl.db.me.planets[coords].resources[res]);
                            });
                        });
...
class MessageManager
{
    ...
    checkCurrentTab()
    {
        ...
        if(this.spyTable)
        {
            ...
            else this.spyTable.classList.add('ogl_hidden');
        }
    }
    ...
    checkRaid()
    {
        ...
        messages.forEach(message =>
        {
            ...
            Util.getXML(`https://${window.location.host}/game/index.php?page=messages&messageId=${id}&tabid=21&ajax=1`, result =>
            {
    ...
    checkReports()
    {
        ...
        {
            setTimeout(() => this.checkReports(), 50);
            return;
        }
        ...
        this.dataList = [];
        if(this.spyTable) this.spyTable.innerHTML = '';
        ...
        messages.forEach((report, index) =>
        {
            ...
            if(compacting.length > 0 && report.querySelector('.msg_title a'))
            {
        ...
        if(this.dataList.length > 0) this.buildSpyTable();
    }
    ...
    getExpeValue(locaAttr, message)
    {
        ...
        if(isResource && message.querySelector('.msg_content').textContent.indexOf(locaAttr + '.') > -1) regex = new RegExp('(\\d+) '+this.cleanString(locaAttr), 'g');
    ...
    buildSpyTable()
    {
        ...
        if(!this.spyTable)
        {
            this.spyTable = Util.createDom('div', {'class':'ogl_spyTable'});
            document.querySelector('ul.subtabs').after(this.spyTable);
        }
        else this.spyTable.textContent = '';
        
        let thead = this.spyTable.appendChild(Util.createDom('div'));
        let tbody = this.spyTable.appendChild(Util.createDom('div'));
        ...
        clean.addEventListener('click', () =>
        {
            this.spyTable.querySelectorAll('div[data-coords]').forEach(line =>
            {
                ...
                {
                    line.querySelector('.ogl_reportOptions div[data-title="delete"]').click();
                }
            });
        });
        ...
        this.dataList.forEach((report, index) =>
        {
            ...
            // date
            let deltaDate =  serverTime.getTime() - timeDiff - report.date;
            let dateObj = new Date(parseInt(report.date));
            ...
            // fleet
            let fleet = content.appendChild(Util.createDom('td', {'class':'ogl_refleet'}, (Util.formatToUnits(report.fleet, 1, true) || '0').replace('-1', '?')))
            ...
            // def
            let def = content.appendChild(Util.createDom('td', {'class':'ogl_redef'}, (Util.formatToUnits(report.defense, 1, true) || '0').replace('-1', '?')));
            ...
            colorContent.addEventListener('click', event =>
            {
                if(report.coords[2] <= 15)
                {
                    ...
                    this.ogl.component.color.addColorUI(colorContent, colors, this.ogl.find(this.ogl.db.positions, 'coords', report.coords.join(':')), event);
                    this.ogl.component.tooltip.open(colorContent, false, false, colors);
                }
            });
...
class KeyboardManager
{
    constructor(ogl)
    {
        ...
        {
            if((this.ogl.mode != 1 && this.ogl.mode != 4) || this.ogl.component.fleet.linksUpdated) window.location.href = this.ogl.prevLink;
        });

        this.addKey('o', this.ogl.component.lang.getText('nextPlanet'), () =>
        {
            if((this.ogl.mode != 1 && this.ogl.mode != 4) || this.ogl.component.fleet.linksUpdated) window.location.href = this.ogl.nextLink;
        });
            ...
            this.addKey('t', this.ogl.component.lang.getText('attackCurrentTarget'), () =>
            {
                ...
                this.ogl.component.sidebar.displayTargetList(() =>
                {
                    if(this.ogl.db.options.nextTargets[0])
                    {
                        ...
                        let maxShips = fleetDispatcher.shipsOnPlanet.find(e => e.id == shipID)?.number || 0;
                        let shipCount = this.ogl.component.fleet.calcRequiredShips(shipID, this.ogl.db.options.rval);
                        fleetDispatcher.selectShip(shipID, Math.min(shipCount, maxShips));
                        ...
                        if(!this.ogl.db.options.nextTargets[1]) this.ogl.db.options.nextTargets[1] = this.ogl.db.options.nextTargets[0];
                    }
                    else
                    {
        ...
        if(this.ogl.page == 'messages')
        {
            ...
            {
                if(document.querySelector('.ogl_spyTable') && document.querySelectorAll('.ogl_spyTable div[data-coords]:not(.ogl_attacked):not(.ogl_notGray)')[0])
                {
                    document.querySelectorAll('.ogl_spyTable div[data-coords]:not(.ogl_attacked):not(.ogl_caution)')[0].querySelector('.ogl_renta a').click();
                }
            });
        }
    }
...
class SidebarManager
{
    ...
    displayPinnedTarget()
    {
        ...
        if(this.ogl.db.pinnedList.length > 0)
        {
            ...
            let planetIndexes = this.ogl.find(this.ogl.db.positions, 'playerID', this.ogl.db.pinnedList[0]);
            planetIndexes.forEach(planetIndex =>
            {
                let planet = this.ogl.db.positions[planetIndex];
                let splitted = planet.coords.split(':');
            ...
            let ptreAction = frame =>
            {
                ...
                Util.getJSON(`https://ptre.chez.gg/scripts/oglight_get_player_infos.php?team_key=${this.ogl.ptre}&pseudo=${player.name}&player_id=${player.id}&input_frame=${frame}`, result =>
                {
                    if(result.code == 1)
                    {
    ...
    displayTargetList(callback)
    {
        if(!callback) this.ogl.db.sidebarView = 'targetList';
        this.ogl.save();

        if(!callback) this.content.textContent = '';
            ...
            for(let s=0; s<500; s+=systemSteps)
            {
                ...
                indexList.forEach(index =>
                {
                    ...
                    if(currentGalaxy == g && currentSystem == s)
                    {
                        ...
                        let playerIndex = this.ogl.find(this.ogl.db.players, 'id', entry.playerID)[0];
                        let player = this.ogl.db.players[playerIndex];
        ...
        if(!callback) this.open(container);
        else callback();
    }
...
class TimeManager
{
    constructor(ogl)
    {
        ...
        {
            document.querySelectorAll('#bar ul li.OGameClock:not(.ogl_ready), #arrivalTime:not(.ogl_ready), #returnTime:not(.ogl_ready)').forEach(element =>
            {
            ...
            document.querySelectorAll('.allianceAttack .arrivalTime:not(.ogl_ready), .eventFleet .arrivalTime:not(.ogl_ready), .fleetDetails .absTime:not(.ogl_ready), .fleetDetails .nextabsTime:not(.ogl_ready)').forEach(element =>
            {
                element.classList.add('ogl_ready');
                this.updateTime(element);
            });
    ...
    checkDetail()
    {
        ...
        if(this.currentDetail && this.currentDetail.querySelector('.og-loading') && this.currentDetail.querySelector('.og-loading').style.display == 'none')
        {
            ...
            if(!this.currentDetail.querySelector('.ogl_detailActions'))
            {
                ...
                let updateLevel = function(newLevel, updateDom)
                {
                    ...
                    ['metal', 'crystal', 'deut', 'energy'].forEach(res =>
                    {
                        if(tech.initial[res])
                        {
                            ...
                            tech.current[res] = Math.ceil(tech.initial[res] * Math.pow(ratio, tech.current.level - tech.initial.level));
                    ...
                    if(tech.id == 1) self.currentDetail.querySelector('.additional_energy_consumption .value').textContent = Util.formatNumber(Math.floor(10 * tech.current.level * Math.pow(1.1, tech.current.level)) - Math.floor(10 * (tech.initial.level - 1) * Math.pow(1.1, tech.initial.level - 1)));
                    else if(tech.id == 2) self.currentDetail.querySelector('.additional_energy_consumption .value').textContent = Util.formatNumber(Math.floor(10 * tech.current.level * Math.pow(1.1, tech.current.level)) - Math.floor(10 * (tech.initial.level - 1) * Math.pow(1.1, tech.initial.level - 1)));
                    else if(tech.id == 3) self.currentDetail.querySelector('.additional_energy_consumption .value').textContent = Util.formatNumber(Math.floor(20 * tech.current.level * Math.pow(1.1, tech.current.level)) - Math.floor(20 * (tech.initial.level - 1) * Math.pow(1.1, tech.initial.level - 1)));
                ...
                {
                    this.currentDetail.querySelector('#build_amount').setAttribute('onkeyup', 'checkIntInput(this, 1, 99999);event.stopPropagation();');
...
class LangManager
{
    constructor(ogl)
    {
        ...
        this.en =
        {
            ...
            oldGalaxy : "Use old galaxy (better perfs ?!)",
        }
        ...
        this.fr =
        {
            ...
            oldGalaxy : "Utiliser l'ancienne galaxie (meilleures perfs ?!)",
        }
...
.ogl_shipIcon.ogl_metal, .ogl_shipIcon.ogl_crystal, .ogl_shipIcon.ogl_deut, .ogl_shipIcon.ogl_dm, .ogl_shipIcon.ogl_energy
{
...
#planetbarcomponent #rechts[data-panel="resources"] #myPlanets .smallplanet a.constructionIcon.moon,
#planetbarcomponent #rechts[data-panel="resources"] #cutty a.constructionIcon.moon
{
    left:96px !important;
}
...
.ogl_statsDetails > div:last-child > div:not(:last-child)
{
    margin-bottom:22px;
}
...
.ogl_dateArea > div > div.ogl_disabled
{
    pointer-events:none;
    opacity:.3;
}
...
{
    background:var(--p3);
    border:6px solid var(--p5);
    border-radius:5px;
    box-shadow:0 0 22px 4px #090b0c;
    box-sizing:border-box !important;
    display:none;
    font-size:11px;
    height:auto;
...
{
    background:var(--p3);
    border-bottom:6px solid var(--p5);
    border-right:6px solid var(--p5);
    bottom:-14px;
    content:'';
    display:block;
    height:15px;
...
.ogl_tooltip .ogl_metal, .ogl_tooltip .ogl_crystal, .ogl_tooltip .ogl_deut
{
    display:inline-block;
    font-size:10px !important;
    font-weight:bold !important;
...
.ogl_tooltip .ogl_metal .value, .ogl_tooltip .ogl_crystal .value, .ogl_tooltip .ogl_deut .value
{
    font-size:10px !important;
    font-weight:bold !important;
}
...
table.fleetinfo:not(.ogl_ignored) tbody
{
    display:grid;
    grid-gap:3px;
    grid-template-columns:repeat(3, 1fr);
...
table.fleetinfo:not(.ogl_ignored) tr
{
    background:var(--p2);
    border-radius:3px;
    color:hsl(208deg 21% 61%);
    display:grid;
    font-size:11px;
    font-weight:bold;
    grid-template-columns:40px auto;
...
table.fleetinfo:not(.ogl_ignored) td.value
{
    font-weight:bold;
    line-height:22px;
    padding:0 5px;
...
table.fleetinfo:not(.ogl_ignored) .ogl_full
{
    grid-column-start:1;
    grid-column-end:4;
    height:7px;
...
table.fleetinfo tr.ogl_activeNames .ogl_shipIcon,
table.fleetinfo tr.ogl_activeNames td.value
{
    border-bottom:1px solid #33414f !important;
}
...
.ogl_pieLabel > span
{
    ...
    font-weight:bold;
    grid-gap:6px;
    grid-template-columns:15px 120px auto 66px;
}
...
.ogl_lockedContainer
{
    ...
    background:var(--p2);
    display:grid;
    grid-template-columns:auto auto auto auto auto 40px 40px 30px;
    line-height:24px;
    padding:16px;
}
...
.ogl_lockedContainer .ogl_button:not(.ogl_header)
{
    border-top:none;
    box-shadow:inset 0 12px rgb(255 255 255 / 7%);
    height:26px;
    line-height:24px !important;
}

.ogl_lockedContainer div.ogl_button:not(.ogl_header)
{
    border-top:none;
}
...
.ogl_mineContainer b.ogl_metal, .ogl_lockedContainer b.ogl_metal { background:hsl(240deg 14% 12%); }
.ogl_mineContainer b.ogl_crystal, .ogl_lockedContainer b.ogl_crystal { background:hsl(205deg 45% 12%); }
.ogl_mineContainer b.ogl_deut, .ogl_lockedContainer b.ogl_deut { background:hsl(170deg 32% 12%); }

.ogl_mineContainer b.ogl_metal.ogl_header, .ogl_lockedContainer b.ogl_metal.ogl_header { background:hsl(240deg 14% 9%) !important; }
.ogl_mineContainer b.ogl_crystal.ogl_header, .ogl_lockedContainer b.ogl_crystal.ogl_header { background:hsl(205deg 45% 9%) !important; }
.ogl_mineContainer b.ogl_deut.ogl_header, .ogl_lockedContainer b.ogl_deut.ogl_header { background:hsl(170deg 32% 9%) !important; }
...
.ogl_share:hover
{
    box-shadow:inset 0 0 6px 3px var(--neon);
}

.ogl_share.ogl_active
{
    box-shadow:inset 0 0 1px 1px hsl(150deg 100% 79%), inset 0 0 6px 3px hsl(155deg 64% 60%);
}
...
#fleet2 .res_wrap:nth-child(1) input,
.ogl_choseCapacity > div:nth-child(2) input
{
    border:2px solid var(--metal) !important;
}

#fleet2 .res_wrap:nth-child(2) input,
.ogl_choseCapacity > div:nth-child(3) input
{
    border:2px solid var(--crystal) !important;
}

#fleet2 .res_wrap:nth-child(3) input,
.ogl_choseCapacity > div:nth-child(4) input
{
    border:2px solid var(--deut) !important;
}
...
#loadAllResources a
{
    background:hsl(28deg 86% 26%) !important;
    border-radius:2px;
    box-shadow:inset 0 12px hsl(8deg 78% 42% / 11%), inset 0 -2px 17px 6px hsl(210deg 30% 9%), 0 0 0 2px var(--p1);
    color:hsl(42deg 63% 55%) !important;
    cursor:pointer;
    font-size:20px;
    height:24px !important;
    line-height:24px !important;
...
.eventFleet[data-mission-type="10"], .fleetDetails[data-mission-type="10"]
{
...
#eventboxContent .eventFleet, #eventboxContent .allianceAttack
{
    ...
    display:grid;
    grid-gap:2px;
    grid-template-columns:85px 72px 23px 95px 70px auto 16px 95px 70px 20px 20px;
    white-space:nowrap;
}
...
.ogl_inFlightTable tr:not(.ogl_full)
{
    background:var(--p2);
}

.ogl_inFlightTable .value
{
    display:table-cell !important;
    padding:3px;
}

.ogl_inFlightTable .ogl_shipIcon
{
    background-position:center;
    display:table-cell !important;
    height:20px;
